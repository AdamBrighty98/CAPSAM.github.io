...
ui <- dashboardPage(skin = "purple",
    ...
    fluidRow(
      ...
      tabBox(title = "Data Input",
         ...
         fileInput("hyd_file", "Select your hydrogen production file: "),
          ...
          tabBox(title = "CSV Template Download",
           tabPanel("Electricity Generation Template Download", 
                    downloadButton("elec_template_download", "Electricity Generation Template", style = "color: #fff; background-color: green; border-color: black")),
           tabPanel("Domestic Heating Template Download", 
                    downloadButton("dh_template_download_pure", "Technology-specific Domestic Heating Template", style = "color: #fff; background-color: green; border-color: black"),
  ...
  tabBox(title = "Results Download",
         downloadButton("download_xlsx", "Download All Results", style = "color = #fff; background-color: red; border-color: black")
         )
  ...
  tabBox(id = "resultsTab", title = "Results", width = 12,
    ...
    tabPanel("Electricity", fluidRow(splitLayout(cellWidths = c("33%", "33%", "33%"), plotlyOutput("elec_co2"), plotlyOutput("elec_nox"), plotlyOutput("elec_pm25")))),
...
#################################################################################################################################################################     
     ...
     data_list <- reactive({
       ...
       list(long_df_elec_co2_emissions,
                                long_df_elec_nox_impacts, 
                                long_df_elec_pm25_impacts,
                                long_df_dh_co2_emissions,
                                long_df_dh_nox_impacts,
                                long_df_dh_pm25_impacts,
                                long_df_ph_co2_emissions,
                                long_df_dh_nox_impacts,
                                long_df_dh_pm25_impacts,
                                long_ironsteel_co2_emissions,
                                long_ironsteel_nox_impacts,
                                long_ironsteel_pm25_impacts,
                                long_hyd_co2_emissions,
                                long_hyd_nox_impacts,
                                long_hyd_pm25_impacts,
                                long_nfm_co2_emissions,
                                long_nfm_nox_impacts,
                                long_nfm_pm25_impacts
                                )
     ...
     output$elec_co2 <- renderPlotly({
      ...
      req(input_elec_file())
       ...
       long_df_elec_co2_emissions <<- melt(df_elec_co2_emissions[, 1:16], id.vars = "Year", variable.name = "Source", value.name = "Emissions", na.rm = TRUE)
        ...
        geom_line() + 
        labs(y = "CO2 Lifecycle Emissions (kt)")
