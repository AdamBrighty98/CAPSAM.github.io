...
ui <- dashboardPage(skin = "purple",
   ...
   fluidRow(
      ...
      tabBox(title = "Parameters", width = 5,
         ...
         # Selecting whether emissons/impacts should be illustrated per unit energy or absolute
          radioButtons("absolute_or_perUnit", "Specify whether emissions/impacts should be stated as absolute or per unit energy", choices = per_unit_or_absolute_choice)
  ...
  column( width = 7,
         ...
         box(title = "Electricity Generation", solidHeader = T, status = "warning", fileInput("elec_file", "Select your electricity generation file: "), downloadButton("elec_template_download", "Electricity Generation Template", style = "color: #fff; background-color: green; border-color: black")),
           
         # Inputting Hydrogen production csv file
         box(title = "Hydrogen Production", solidHeader = T, status = "warning", fileInput("hyd_file", "Select your hydrogen production file: "),
             downloadButton("hyd_template_download", "Hydrogen Production Template", style = "color: #fff; background-color: green; border-color: black")),
         
         # Inputting domestic heat csv input
         box(title = "Domestic Heating", solidHeader = T, status = "warning", fileInput("dh_file", "Select your domestic combustion file: "),
         ...
         box(title = "Public & Commericial Heating", solidHeader = T, status = "warning", fileInput("ph_file", "Select your public and commercial combustion file: "),
         ...
         box(title = "Industrial Combustion", solidHeader = T, status = "warning", fileInput("ind_file", "Select your industrial combustion file: "),
  ...
  # Results Download Box
    ...
    tabBox(title = "Results Download", width = 12,
         ...
         tabPanel("Public and Commercial Results Download", downloadButton("download_ph_xlsx", "Download Public and Commercial Heating Results", style = "color = #fff; background-color: red; border-color: black")),
...
server <- function(input, output, session) {
  ...
  # Reading and transposing input csv files
     ...
     # nfm_data now on rows 1:6 (inc electricity)! Pick 2:6 for all industry sums
     
     nfm_data_sum <- reactive({
       
       req(nfm_data())
       
       nfm_data_sum <- rowSums(nfm_data()[2:6], na.rm = TRUE)
     ...
     output$elec_co2 <- renderPlotly({
      ...
      req(input_elec_file())
       ...
       long_df_elec_co2_emissions <- melt(df_elec_co2_emissions[, 1:16], id.vars = "Year", variable.name = "Source", value.name = "Emissions", na.rm = TRUE)
        ...
        geom_line() + 
        labs(y = "CO2e Lifecycle Emissions (kt)")
       ...
       long_df_elec_co2_emissions <- melt(df_elec_co2_emissions[, 1:16], id.vars = "Year", variable.name = "Source", value.name = "Emissions", na.rm = TRUE)
        ...
        p <- ggplot(long_df_elec_co2_emissions,
               aes(Year, Emissions, col = Source)) +
        geom_line() + 
        labs(y = "CO2e Lifecycle Emissions per TWh (kt/TWh)")
     ...
     # NOx PWMC Impacts (currently not weighted per unit energy)
      ...
      req(input_elec_file())
       ...
       long_df_elec_nox_impacts <- melt(df_elec_nox_impacts[, 1:9], id.vars = "Year", variable.name = "Source", value.name = "PWMC", na.rm = TRUE)
        ...
        p <- ggplot(long_df_elec_nox_impacts, aes(Year, PWMC, col = Source)) +
        geom_line() + 
        labs(y = "NOx Impact per TWh (ng m-3 TWh-1)")
...
#########################################################################################################
     ...
     output$elec_pm25 <- renderPlotly({
      ...
      req(input_elec_file())
       ...
       long_df_elec_pm25_impacts <- melt(df_elec_pm25_impacts[, 1:9], id.vars = "Year", variable.name = "Source", value.name = "PWMC", na.rm = TRUE)
        ...
        p <- ggplot(long_df_elec_pm25_impacts, aes(Year, PWMC, col = Source)) +
        geom_line() + 
        labs(y = "PM2.5 Impact per TWh (ng m-3 TWh-1)")
     ...
     output$dh_co2 <- renderPlotly({
       ...
       p <- ggplot(long_df_dh_co2_emissions, aes(Year, Emissions, col = Source)) +
                geom_line() + 
                labs(y = "CO2 Emissions per TWh (kt TWh-1)")
     ...
     output$dh_nox <- renderPlotly({
       ...
       p <- ggplot(long_df_dh_nox_impacts, aes(Year, Impacts, col = Source)) +
                geom_line() + 
                labs(y = "NOx Impacts per TWh (ng m-3 TWh-1)")
     ...
     output$dh_pm25 <- renderPlotly({
       ...
       p <- ggplot(long_df_dh_pm25_impacts, aes(Year, Impacts, col = Source)) +
                geom_line() + 
                labs(y = "PM2.5 Impacts per TWh (ng m-3 TWh-1)")
    ...
    ####################################################################################################################
     ...
     output$ironsteel_nox <- renderPlotly({
        ...
        ironsteel_nox_impacts$`Total Impact` <- rowSums(ironsteel_nox_impacts[ , 2:5], na.rm = TRUE)
        ...
        ironsteel_nox_impacts$`Total Impact per TWh` <- rowSums(ironsteel_nox_impacts[ , 2:5], na.rm = TRUE)
    ...
    ####################################################################################################################
     ...
     output$ironsteel_pm25 <- renderPlotly({
        ...
        ironsteel_pm25_impacts$`Total Impact` <- rowSums(ironsteel_pm25_impacts[ , 2:5], na.rm = TRUE)
        ...
        ironsteel_pm25_impacts$`Total Impact per TWh` <- rowSums(ironsteel_pm25_impacts[ , 2:5], na.rm = TRUE)
    ...
    ####################################################################################################################
       ...
       output$hydrogen_co2 <- renderPlotly({
         ...
         p <- ggplot(long_hyd_co2_emissions, aes(Year, Emissions, col = Source)) +
           geom_line() + 
           labs(y = "CO2e Lifecycle Emissions per TWh (kt TWh-1)")
  ...
  ####################################################################################################################
     ...
     # Non-ferrous metals CO2 emissions
      ...
      output$nfm_co2 <- renderPlotly({
        ...
        nfm_co2_emissions$`Total Emissions per TWh` <- rowSums(nfm_co2_emissions[ , 2:5], na.rm = TRUE)
    ...
    ####################################################################################################################
     ...
     output$nfm_nox <- renderPlotly({
        ...
        nfm_nox_impacts$`Total Impact` <- rowSums(nfm_nox_impacts[ , 2:5], na.rm = TRUE)
    ...
    ####################################################################################################################
     ...
     output$nfm_pm25 <- renderPlotly({
        ...
        nfm_pm25_impacts$`Total Impact` <- rowSums(nfm_pm25_impacts[ , 2:5], na.rm = TRUE)
        ...
        nfm_pm25_impacts$`Total Impact per TWh` <- rowSums(nfm_pm25_impacts[ , 2:5], na.rm = TRUE)
